...
checkpoint_path = os.path.join(RESULTS, 'checkpoints/lastro_cnn/lastro_cnn.ckpt')
...
es_callback = tf.keras.callbacks.EarlyStopping(monitor='val_acc', min_delta=1, patience=5, verbose=0, mode='auto', baseline=None, restore_best_weights=True)
...
model.save(os.path.join(RESULTS,'lastro_cnn.h5'))
with open(os.path.join(RESULTS,'lastro_cnn.history'), 'wb') as file_pi:

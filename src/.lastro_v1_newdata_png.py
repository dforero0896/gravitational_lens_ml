...
def main():
    ...
    # Create ImageDataGenerator objects to inherit random transformations from Keras' class.
    image_data_gen_train = ImageDataGenerator(featurewise_center=False,
                                                rotation_range=0,
                                                fill_mode='wrap',
                                                ...
                                                dtype='float32')
    image_data_gen_val = ImageDataGenerator(dtype='float32',
                                            preprocessing_function=preprocess_band)
    # Create Generator objects from the initialized ImageDataGenerators.
    # To train
    train_data_gen = image_data_gen_train.flow_from_directory(directory=os.path.join(DATA, 'elodiedata/train'), batch_size=batch_size, color_mode='grayscale', class_mode='binary', seed=42, shuffle=True)
    # To validate
    val_data_gen = image_data_gen_val.flow_from_directory(os.path.join(DATA, 'elodiedata/val'), color_mode='grayscale', batch_size=batch_size, class_mode='binary', shuffle=True, seed=42)
    # To predict/evaluate                                                            
    roc_val_data_gen = image_data_gen_val.flow_from_directory(os.path.join(DATA, 'elodiedata/val'), color_mode='grayscale', batch_size=batch_size, class_mode='binary', shuffle=True, seed=42)
    # To safely obtain image size
    temp_data_gen = image_data_gen_train.flow_from_directory(os.path.join(DATA, 'elodiedata/val'), color_mode='grayscale', batch_size=1, class_mode='binary', shuffle=True, seed=42)
